...
public class MainActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        settingsButton.setOnClickListener(v -> showSettings()); homeNavButton.setOnClickListener(v -> setFilter("all"));
        favoritesNavButton.setOnClickListener(v -> setFilter("favorites")); settingsNavButton.setOnClickListener(v -> showSettings());
        searchBox.addTextChangedListener(new TextWatcher() {
            @Override public void beforeTextChanged(CharSequence s, int start, int count, int after) {}
            @Override public void onTextChanged(CharSequence s, int start, int before, int count) { buildStationList(s == null ? "" : s.toString()); }
            @Override public void afterTextChanged(Editable s) {}
        });
        updateModeButtons(); updateViewIcon();
    }
    ...
    private void applyLanguage() {
        boolean rtl="ar".equals(lang); root.setLayoutDirection(rtl?LinearLayout.LAYOUT_DIRECTION_RTL:LinearLayout.LAYOUT_DIRECTION_LTR);
        appTitle.setText("Radio Maroc"); appSubtitle.setText(t("صوت المغرب أينما كنت","La voix du Maroc, partout avec vous","Moroccan voices, everywhere with you"));
        searchBox.setHint(t("ابحث عن محطة…","Rechercher une station…","Search stations…")); allButton.setText(t("الكل","Toutes","All"));
        favoritesButton.setText(t("المفضلة","Favoris","Favorites")); recentButton.setText(t("الأخيرة","Récentes","Recent"));
    ...
    private void buildStationList(String query){
        stationsGrid.removeAllViews(); boolean grid="grid".equals(viewMode),compact="compact".equals(viewMode); stationsGrid.setColumnCount(grid?2:1);
        ...
        for(Station station:Stations.ALL){
            ...
            GridLayout.LayoutParams cp=new GridLayout.LayoutParams();cp.setMargins(dp(4),dp(4),dp(4),dp(4));cp.height=ViewGroup.LayoutParams.WRAP_CONTENT;
            if(grid){cp.width=0;cp.columnSpec=GridLayout.spec(GridLayout.UNDEFINED,1f);}else{cp.width=ViewGroup.LayoutParams.MATCH_PARENT;cp.columnSpec=GridLayout.spec(0,1f);}card.setLayoutParams(cp);
            ImageView logo=new ImageView(this);int logoSize=dp(grid?96:(compact?42:58));LinearLayout.LayoutParams lp=new LinearLayout.LayoutParams(logoSize,logoSize);
    ...
    private TextView actionText(String text,int color){TextView v=new TextView(this);v.setText(text);v.setTextSize(23);v.setTextColor(getColor(color));v.setGravity(Gravity.CENTER);v.setPadding(dp(8),0,dp(8),0);return v;}
    ...
    private void showAboutDialog(){
        String msg=t(
            "راديو المغرب تطبيق مجاني ومفتوح المصدر وبدون إشهارات، أنشئ بهدف الخدمة فقط. المبرمج M@ster لا يطلب من المستخدمين أي أموال أو تبرعات أو تحويلات مالية بأي شكل. أي شخص يطلب المال باسم التطبيق أو المبرمج لا يمثل راديو المغرب. التطبيق يهدف إلى توفير وسيلة بسيطة وآمنة للاستماع إلى الإذاعات المغربية، خصوصاً للجالية المغربية بالخارج.\n\nسيُضاف لاحقاً عنوان تواصل مستقل خاص بالتطبيق للملاحظات والاقتراحات، دون نشر البريد الشخصي للمبرمج.",
            "Radio Maroc est une application gratuite, open source et sans publicité, créée uniquement dans un but de service. Le développeur M@ster ne demande jamais d’argent, de dons ni de virements aux utilisateurs. Toute personne demandant de l’argent au nom de l’application ou du développeur ne représente pas Radio Maroc. L’application vise à offrir un moyen simple et sûr d’écouter les radios marocaines, notamment pour les Marocains résidant à l’étranger.\n\nUne adresse de contact indépendante dédiée à l’application sera ajoutée ultérieurement pour les remarques et suggestions, sans publier l’adresse personnelle du développeur.",
            "Radio Maroc is free, open-source and ad-free, created solely as a public-service project. Developer M@ster never asks users for money, donations or financial transfers. Anyone requesting money in the name of the app or developer does not represent Radio Maroc. The app provides a simple and secure way to listen to Moroccan radio, especially for Moroccans living abroad.\n\nA separate app-only contact address will be added later for feedback and suggestions without publishing the developer’s personal email."
        );
    ...
    private void showPrivacyDialog(){String msg=t("لا إعلانات، لا تحليلات، لا تتبع، ولا جمع للبيانات الشخصية. الكود مفتوح المصدر ويمكن تدقيقه. لا يُستخدم HTTP غير المشفر للبث في هذه النسخة.","Aucune publicité, analytique, traçage ni collecte de données personnelles. Le code est open source et vérifiable. Aucun flux HTTP non chiffré n’est utilisé dans cette version.","No ads, analytics, tracking, or personal-data collection. The source code is open for audit. This build does not use unencrypted HTTP streams.");new AlertDialog.Builder(this).setTitle("🔒  "+t("الخصوصية والأمان","Confidentialité et sécurité","Privacy & security")).setMessage(msg).setPositiveButton("OK",null).show();}
